#ifndef XSTD_BASE_ARRAY_HPP
#define XSTD_BASE_ARRAY_HPP
...
#include <xstd/bit.hpp>         // countl_zero, countr_zero, popcount, 
                                // bit_intersects, bit_is_subset, bit_not_equal_to
...
public:
        [[nodiscard]] constexpr base_array() noexcept = default;

        [[nodiscard]] friend constexpr bool operator==(base_array const& x [[maybe_unused]], base_array const& y [[maybe_unused]]) noexcept
        {
                if constexpr (N == 0) {
                        return true;
                } else {
        ...
        [[nodiscard]] friend constexpr auto operator<=>(base_array const& x [[maybe_unused]], base_array const& y [[maybe_unused]]) noexcept
                -> std::strong_ordering
        {
                if constexpr (N == 0) {
        ...
        [[nodiscard]] constexpr std::size_t find_front() const noexcept
        {
                ...
                if constexpr (num_blocks == 1) {
                        return countr_zero(m_bits[0]);
                } else if constexpr (num_blocks == 2) {
                        return m_bits[0] != zero ? countr_zero(m_bits[0]) : countr_zero(m_bits[1]) + bits_per_block;
                        ...
                        return countr_zero(*front) + std::ranges::subrange(std::ranges::begin(m_bits), front).size() * bits_per_block;
                }
        }
        ...
        [[nodiscard]] constexpr std::size_t find_back() const noexcept
        {
                assert(any());
                if constexpr (num_blocks == 1) {
                        return last_bit - countl_zero(m_bits[0]);
                } else if constexpr (num_blocks == 2) {
                        return m_bits[1] != zero ? last_bit - countl_zero(m_bits[1]) : left_bit - countl_zero(m_bits[0]);
                } else if constexpr (num_blocks >= 3) {
                        ...
                        return last_bit - std::ranges::subrange(std::ranges::rbegin(m_bits), back).size() * bits_per_block - countl_zero(*back);
                }
        }
        ...
        [[nodiscard]] constexpr std::size_t find_first() const noexcept
        {
                if constexpr (N > 0 and num_blocks == 1) {
                        ...
                        if (m_bits[0] != zero) {
                                ...
                                return countr_zero(m_bits[1]) + bits_per_block;
                        }
                } else if constexpr (num_blocks >= 3) {
                        if (
                                ...
                                return countr_zero(*first) + std::ranges::subrange(std::ranges::begin(m_bits), first).size() * bits_per_block;
                        }
                }
                return N;
        }
        ...
        [[nodiscard]] constexpr std::size_t find_next(std::size_t n) const noexcept
        {
                ...
                if constexpr (num_blocks == 1) {
                        ...
                        if (
                                ...
                                return countr_zero(*next) + std::ranges::subrange(std::ranges::begin(rg), next).size() * bits_per_block + n;
                        }
                }
                return N;
        }
        ...
        [[nodiscard]] constexpr std::size_t find_prev(std::size_t n) const noexcept
        {
                ...
                --n;
                if constexpr (num_blocks == 1) {
                        return n - countl_zero(static_cast<Block>(m_bits[0] << (left_bit - n)));
                } else if constexpr (num_blocks >= 2) {
                        auto [ index, offset ] = index_offset(n);
                        ...
                        return n - std::ranges::subrange(std::ranges::begin(rg), prev).size() * bits_per_block - countl_zero(*prev);
                }
        }

        constexpr void operator&=(base_array const& other [[maybe_unused]]) noexcept
        {
                if constexpr (N > 0 and num_blocks == 1) {
        ...
        constexpr void swap(base_array& other [[maybe_unused]]) noexcept(std::is_nothrow_swappable_v<Block>)
        {
                if constexpr (N > 0) {
        ...
        [[nodiscard]] constexpr std::size_t count() const noexcept
        {
                if constexpr (N == 0) {
                        ...
                        return popcount(m_bits[0]);
                } else if constexpr (num_blocks == 2) {
                        return popcount(m_bits[0]) + popcount(m_bits[1]);
                } else if constexpr (num_blocks >= 3) {
                        return std::ranges::fold_left(
                                m_bits | std::views::transform([](auto block) { return popcount(block); }), 
                                0uz, std::plus<>()
                        );
                }
        }
        ...
        [[nodiscard]] constexpr bool is_subset_of(base_array const& other [[maybe_unused]]) const noexcept
        {
                if constexpr (N == 0) {
                        return true;
                } else if constexpr (num_blocks == 1) {
        ...
        [[nodiscard]] constexpr bool is_proper_subset_of(base_array const& other [[maybe_unused]]) const noexcept
        {
                if constexpr (N == 0) {
                        return false;
                } else if constexpr (num_blocks == 1) {
        ...
        [[nodiscard]] constexpr bool intersects(base_array const& other [[maybe_unused]]) const noexcept
        {
                if constexpr (N == 0) {
                        return false;
                } else if constexpr (num_blocks == 1) {
