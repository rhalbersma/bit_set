...
concept bit_range = 
        ...
        {
                { find_at(c, n) } -> std::convertible_to<bool>;
        }
;
...
template<bit_range Bits> [[nodiscard]] constexpr iterator<Bits, false> begin(      Bits& c) noexcept { return { &c, find_first(c) }; }
template<bit_range Bits> [[nodiscard]] constexpr iterator<Bits, true > begin(const Bits& c) noexcept { return { &c, find_first(c) }; }
template<bit_range Bits> [[nodiscard]] constexpr iterator<Bits, false> end  (      Bits& c) noexcept { return { &c, find_last (c) }; }
template<bit_range Bits> [[nodiscard]] constexpr iterator<Bits, true > end  (const Bits& c) noexcept { return { &c, find_last (c) }; }
